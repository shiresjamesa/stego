...
NEW_FILE = "photos/outputP.png"
PASSWORD = b'\x01\x02\x03'
...
def test(pix):
    a = list(pix[0,0]) #TODO Not needed if loaded correctly
    a[0] = int(bin(pix[0,0][0])[2:-1]+"1",2)
    pix[0,0] = tuple(a)

def testR(pix):
    a = list(pix[0,0]) #TODO Not needed if loaded correctly
    a[0] = int(bin(pix[0,0][0])[2:-1]+"0",2)
    pix[0,0] = tuple(a)
    return pix
